...
- **処理の流れ**: 該当Matchの`settings`(精算時点。旧データはグループ現在設定にフォールバック)で結果表示を組み立て、半荘ごとの内訳・グラフを表示。グラフ画像に「この対戦を再オープン」(下記15)・「この対戦を削除」(`_drop_m_confirm?to=`→確認の「削除する」で`_drop_m_select?to=`)のQuick Replyを付ける
- **結果**: なし
