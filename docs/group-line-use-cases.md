...
- **トリガー**: A。グループ参加時に自動送信(上記1) + 精算確認メニュー「いいえ」ボタン(`data="_start"`) + `_start`手打ちでも可
- **前提条件**: なし
- **処理の流れ**: `reply_service.add_start_menu()`を呼ぶのみ。「結果を入力」「精算」「設定」「その他」の4ボタンを提示
- **結果**: ボタン付きメッセージ1通
...
- **トリガー**: **C**。ボタン導線なし。open対戦2件以上時のピッカーに「新しい対戦を始める」ボタン(`data="_new_match"`)はあるが、**open対戦0〜1件の状態から`_new_match`自体を最初に呼ぶボタンは存在しない**
- **前提条件**: `Group.mode != input`
...
- **トリガー**: A。「その他」メニュー「途中経過を確認」(`_active_match`) + open対戦2件以上時のピッカー(`_active_match_select?to=`)
- **前提条件**: なし(0件時は専用メッセージ)
...
- **トリガー**: A。「その他」メニュー「成績推移」(`_history_start`)→対象選択ボタン(自分だけ/グループ全員/選択する、`_history_target?t=`)→(選択する時のみ)ユーザーカルーセルでトグル選択(`_history_toggle?u=`)→確定ボタン(`_history_confirm`)→期間選択ボタン(今月/先月/3ヶ月/6ヶ月/全期間、`_history_exec?p=`)
- **前提条件**: 「選択する」を選んだ場合、最低1人以上選択が必要(0人のまま確定しようとするとカルーセル再提示)
- **処理の流れ**: 各ステップは`HistorySession`(`line_group_id`+`requester_line_id`キー、per-user状態)に選択状態を保存しながら進行。セッションタイムアウト時(`HistorySession`が見つからない)は全ステップで「タイムアウトしました。その他メニューの「成績推移」から再度お試しください。」。最終ステップで対象ユーザーの`UserMatch`から該当Matchを集計し、累計推移の折れ線グラフを生成
...
## 21. 成績推移を見る(メンション一括指定、簡易版)

- **トリガー**: **C**。`_history`(+メンションで対象を直接指定。例:「@Alice @Bob _history」)。上記20の5ステップフローとは別の独立した実装で、ボタン導線なし
- **前提条件**: なし(メンションなしなら送信者本人のみが対象)
- **処理の流れ**: メンションされたユーザー+送信者自身を対象に`UserMatch`から該当Matchを集計し、その場で累計推移グラフを1回で生成(セッション状態を持たない一発呼び出し)。`@All`メンション時はグループの対戦参加歴がある全ユーザーが対象
- **結果**: なし
- **関連ファイル**: `src/use_cases/group_line/reply_multi_history_use_case.py`

## 22. 累計得点表・順位表を見る

- **トリガー**: **C**。`_ranking`(+任意でメンション指定、未指定時は送信者本人)
- **前提条件**: なし
- **処理の流れ**: メンションされたユーザー(+送信者)を対象に累計スコア・平均順位・最高得点等を集計し、LINEプロフィール画像付きの画像テーブルを2枚(得点表・順位表)生成
- **結果**: なし
- **関連ファイル**: `src/use_cases/group_line/reply_ranking_table_use_case.py`

## 23. 個人の順位履歴・順位分布を見る

- **トリガー**: **C**。`_rank`(順位履歴グラフ+平均順位)、`_rank_detail`(順位ヒストグラム)。ボタン導線なし
- **前提条件**: なし
...
- **トリガー**: **B**。`_sim`(手打ちで開始)→点数入力(テキスト、ボタンなし)
- **前提条件**: `Group.mode != sim`(既にsimモードなら「すでにシミュレーションモードです。」)
...
## 32. Botへのメンションでスタートメニューを再表示する(実装中・新機能)

- **トリガー**: A(実装方針)。メンション(`is_self=True`のmentioneeを含む)。本ドキュメント作成時点でブランチ`fezzlk796/mention-start-menu`に未コミットで実装中
...
## 未接続ユースケース(コードは存在するがdispatchテーブル未登録、呼び出し不可)

### A. 対戦単位の削除(`_drop_m`)

`routing_by_text_in_group_line.py`内のコメントで「not yet implemented」と明記。`DropMatchByIndexUseCase`は実装済みで、`_matches`一覧のN番目を指定して対戦(=半荘の集合)ごと`is_deleted=True`にする処理を持つが、dispatchテーブルに未登録のため現状呼び出せない。FEZ-137で対応要否判断待ち。
関連ファイル: `src/use_cases/group_line/drop_match_by_index_use_case.py`

### B. 対戦横断の合計集計(`_sum_matches`)

`reply_sum_matches_by_ids_use_case.py`は中身がコメントのみ(`# ReplySumMatchesByIdsUseCase is not yet implemented`)で未実装。FEZ-137で対応要否判断待ち。

### C. 素点一覧の貼り付けによる半荘一括登録(`_add_result`相当)

`AddHanchanByPointsTextUseCase`は実装済みだが、全文検索してもファイル自身の定義以外どこからも参照されておらず、dispatchテーブルにも未登録。`名前: 点数`形式のテキストを複数行貼り付けると4人分揃った時点で`SubmitHanchanUseCase`を呼ぶ処理を持つ。PR #272(`feat/paste-raw-score-reply`ブランチ、本ドキュメント作成時点で未マージ)がこの機能の完成版を含んでいる可能性が高い(要確認: mainの本ファイルとPR #272の差分は未比較)。
...
- PR #272の`AddHanchanByPointsTextUseCase`実装がこのドキュメント記載のmain版と同一か、機能追加されているかは未比較
