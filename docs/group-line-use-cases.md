...
| 設定(`_setting`) | FlexCarousel(3枚) | 「精算ルール」: レート・順位点・チップ・飛び賞・端数計算方法 / 「メンバー」: 人数・ゲスト / 「その他」: 他グループへ統合・ヘルプ |
| 対戦管理(`_others`) | FlexCarousel(2枚) | 「進行中の対戦」: 途中経過を確認・新しい対戦を始める・シミュレーション / 「戦績」: 成績推移・対戦履歴・累計得点表・順位表・個人の順位推移・個人の順位分布 |

画面に埋め込むボタン(独立したメニュー項目にしない):
- 対戦履歴一覧: 直近10件の詳細表示 + 「まとめて精算」(Quick Reply)
- 対戦詳細: 「この対戦を再オープン」「この対戦を削除」(Quick Reply)
...
- **前提条件**: なし
- **処理の流れ**: `status=settled`かつ`is_deleted=False`のMatch一覧取得(0件なら「まだ対戦結果がありません。」)→複数系列使用歴があれば対戦名を、なければ日付のみのリストを表示。直近10件の詳細表示ボタンと「まとめて精算」(`_sum_matches`)をQuick Replyで提示
- **結果**: なし
- **関連ファイル**: `src/use_cases/group_line/reply_matches_use_case.py`

## 13. 対戦の詳細を見る

- **トリガー**: A。上記12の一覧のQuick Reply(`_match_select?to=<match_id>`)。`_match N`の手打ちも後方互換で残す
- **前提条件**: 対象が`status=settled`(実行時に再検証)
- **処理の流れ**: 該当Matchの`settings`(精算時点。旧データはグループ現在設定にフォールバック)で結果表示を組み立て、半荘ごとの内訳・グラフを表示。グラフ画像に「この対戦を再オープン」(下記15)・「この対戦を削除」(`_drop_m_select?to=`)のQuick Replyを付ける
- **結果**: なし
- **関連ファイル**: `src/use_cases/group_line/reply_match_by_index_use_case.py`

## 14. 現在の対戦の半荘を削除する

- **トリガー**: A。上記11の途中経過のQuick Reply(`_drop_select?to=<hanchan_id>`、直近10件)。`_drop N`の手打ちも後方互換で残す
- **前提条件**: 対象半荘が未削除で、紐づく対戦がこのグループの`status=open`(実行時に再検証)
- **処理の流れ**: 該当Hanchanを`is_deleted=True`に更新(物理削除ではない)。削除完了メッセージに続けて、削除した半荘の素点を「名前: 点数」形式(素点一覧の貼り付け登録がそのまま受け付ける形式)で送り、コピペで再登録できると案内する(名前を解決できない参加者がいる場合は省略)
- **結果**: Hanchan.is_deleted=True
- **関連ファイル**: `src/use_cases/group_line/drop_hanchan_by_index_use_case.py`

## 15. 対戦(系列)自体を再オープンする

- **トリガー**: A。上記13の対戦詳細の「この対戦を再オープン」(`_reopen_confirm?to=`)。`_reopen`の手打ち→直近10件からのQuick Reply選択も残す
- **前提条件**: 対象が`status=settled`(実行時に再検証、FEZ-226)
...
- **トリガー**: A。スタートメニュー「設定」(`_setting`)→設定カルーセルの各項目→`_update_config <key> <value>`(ボタンのdata値として送信されるため実質ボタン)
- **前提条件**: なし
- **処理の流れ**: `_setting`(body="")で現在設定一覧を表示しつつ設定カルーセルを提示(旧ページ切替キー`メニュー1`/`メニュー2`も同じカルーセルを返す)。各項目ボタンでサブメニュー(値の選択肢)を表示、選択すると`_update_config`が呼ばれ値を検証(レート:0/1/2/3/4/5/10、チップ:0/1、飛び賞:0/10/20/30、順位点:人数分のカンマ区切り、端数計算方法:定義済みリスト内、単位:10文字以内の任意文字列)して`GroupSetting`を更新
...
- **トリガー**: A。設定カルーセル「人数」(`_setting 人数`)→Quick Reply「4人」「3人」(`_update_config 人数 4|3`)
- **前提条件**: `value`が3または4
...
- **トリガー**: A。設定カルーセル「ゲスト」(`_setting ゲスト`)→ゲスト管理メニュー「追加」ボタン(`_guest_add`)
- **前提条件**: なし
...
- **トリガー**: A。設定カルーセル「他グループへ統合」(`_migrate`)→統合先グループ選択Quick Reply(`_migrate_confirm?to=`はボタン)。個人DM版は`_personal_migrate`(パラメータ有無で3ステップ、統合元・統合先ともボタン選択、起点のみ手打ち)
...
- **トリガー**: A。結果入力・シミュレーションの開始メッセージの「中断する」(`_exit`)。ソースコード上「danger」とコメントされている破壊的操作
- **前提条件**: グループが登録済み
...
FEZ-137で実装済み。`_drop_m`は上記13の対戦詳細、`_sum_matches`は上記12の対戦履歴一覧から呼ぶ(直近10件の精算済み対戦から複数選択→プレイヤーごとの金額を合算して表示。精算のやり直しではない)。素点一覧の貼り付け登録はPR #272でmainにマージ済み(`AddHanchanByPointsTextUseCase`、コマンドなしで自動判定)。`_add_result`はenumごと廃止。
