...
class ReplyService(IReplyService):
    ...
    def add_others_menu(self) -> None:
        self.buttons.append(
            TemplateMessage(
                ...
                # ButtonsTemplateのactionsは最大4件のため、追加機能は同じ
                # メッセージにQuick Replyとして付与する(スタートメニューの
                # 「新しい対戦を始める」と同じ手法)
                quick_reply=QuickReply(
                    items=[
                        QuickReplyItem(
    ...
    def build_drop_match_quick_reply(self, match) -> QuickReply:
        """対戦自体を削除するボタンのQuick Replyを構築して返す。

        戻り値は呼び出し元がadd_image(quick_reply=...)等、実際に送信される
        メッセージ列の最後の1件に渡すこと(build_drop_target_quick_reply参照)。
        """
        label = "この対戦を削除"
        return QuickReply(
            items=[
